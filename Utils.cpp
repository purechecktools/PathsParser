...
std::vector<std::string> getAllTargetPaths() {
    ...
    for (auto& fn : targetFiles) {
        std::string full = "C:\\" + fn;
        if (file_exists(full)) {
            std::cout << "Found and reading: " << full << std::endl;
            auto v = readPathsFromFile(full);
            allPaths.insert(allPaths.end(), v.begin(), v.end());
        }
    }
